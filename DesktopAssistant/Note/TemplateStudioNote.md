...
ラジオボタンは何もバインドしてなくても同じグループのボタンクリックで選択状態が解除されるが、そのクリックコマンドでバインドした変数を動かすと同時更新になってしまう。
コマンドはバインド変数更新後に実行されるので、二重に処理しないように。  
→と思ったけど直らん。IsCheckedのバインドが何かしらのバグで時々エラーを出すので使わないようにしよう。詳細画面から「戻る」で遷移したときに落ちるからどうしようもない。

## DateTime.UtcNowで登録した時刻を表示するとちゃんとローカル時刻になってる。不思議。
何で？？いいの？